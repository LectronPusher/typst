...
// The parser creates the main syntax structure, but its job isn't
// really to check and verify everything, it mainly prepares SyntaxNodes
// as SyntaxKind elements with children nested in a vector.
// Yhen in `crates/typst/src/eval`, these nodes are cast into the
// respective ast node based on dispatch to the SyntaxNode::cast::<T>() call.
// This is the implicit contract between syntax, the ast, and eval.
// Eval calls cast::<T> to find the type of a node in the ast,
// which figures this out by checking the SyntaxKind of the outermost SyntaxNode
// and then eval recurses into the single AstNode, calling cast every
// time it needs to figure out what its next node is.

// ah, found the reason for this design:
// https://matklad.github.io/2023/05/21/resilient-ll-parsing-tutorial.html
// this is really required reading imo
// found from: https://github.com/typst/typst/pull/2091
// hmm, they also link to:
...
mod tests {
    use super::*;
    // run with: `cargo test -p typst-syntax -- -q --nocapture ian`
...
/// Parses a flat math expression leaving the actual nested parsing for later
/// evaluation.
///
/// This is required for a number of tricky parsing details that
/// can't be known until runtime. i.e. Whether an ident followed by parentheses
/// is a function call or not. This allows us to split the precedence so
/// subscripts parse `f_pi(x)` as `f_pi (x)` vs `f_(pi(x))`.
fn math_expr_flat(p: &mut Parser) {
    loop {
        // Note: It's important that we always eat in every branch, otherwise we
        // could cause an infinite loop.
        match p.current() {
            SyntaxKind::Dollar | SyntaxKind::Eof => return,
            ...
            | SyntaxKind::Str
            | SyntaxKind::Root
            | SyntaxKind::Underscore // Note: Can't start a math expression
            | SyntaxKind::Slash      // Note: Can't start a math expression
            | SyntaxKind::Hat        // Note: Can't start a math expression
            | SyntaxKind::Text => {
                p.eat()
            }
            // Group primes so that we can combine them smartly later.
            SyntaxKind::Prime => {
                let m = p.marker();
                while p.directly_at(SyntaxKind::Prime) {
                    p.eat()
                }
                p.wrap(m, SyntaxKind::MathPrimes)
            }
            // Parse a code expression in math
            SyntaxKind::Hash => {
                embedded_code_expr(p);
            }
            // TODO: should we do anything here?
            SyntaxKind::Error => {} // ??? should we eat ???
            // This shouldn't happen: no other types should be generated by the lexer
            _ => p.expected("math expression"),
        }
    }
    // This is eventually wrapped in a SyntaxKind::Math by the caller
}
...
fn math_expr_prec(p: &mut Parser, min_prec: usize, stop: SyntaxKind) {
    ...
    // This is the start of the expression, these match exactly (top to bottom)
    // the values in the MATH_EXPR SyntaxSet (in set.rs)
    // Once we've gotten started, we can continue with the more complicated parsing
    // i.e. we can't parse operators before we have something else first
    match p.current() {
        SyntaxKind::Hash => embedded_code_expr(p),
        SyntaxKind::MathIdent => {
            continuable = true;
    ...
    // Once we have the start, try to enclose it in delimiters I guess????
    // This [1] commit seems to be where "continuable" came from, but unfortunately
    // it was before the open-sourcing, so no associated pull-request that I can see.
    // [1] https://github.com/typst/typst/commit/2bb0135d2ad45b2a7228dbb61a04a703b1942798
    if continuable
        && min_prec < 3
        && p.prev_end() == p.current_start()
        && maybe_delimited(p)
    {
        // not sure why we do this wrapping
        p.wrap(m, SyntaxKind::Math);
    }
    // Note: Currently this wraps an unclosed open paren in an EXTRA Math node

    // Whether there were _any_ primes in the loop.
    let mut primed = false;

    // Now we actually try to parse operators and such
    while !p.eof() && !p.at(stop) {
        ...
        match assoc {
            ...
            // this is why min_prec < 3 matters maybe?? (root is 2 and left)
        ...
        p.eat();
        let m2 = p.marker();
        // note: not the same stop we started with
...
fn math_delimited(p: &mut Parser) {
    ...
    // this is needed for math_unparen when handling fractions with unbalanced parens
    // i.e. $ 1/(2 (x) $ should render as frac( $1$, $(2 (x)$ )
    // the unfinished delimiter needs to slurp up all of its neighbors
    // idea: change math_unparen to not slurp unfinished delimiters
    // then comment this out
    p.wrap(m, SyntaxKind::Math);
}

// Unsent discord comment:
// Hi, still looking into doing runtime parsing changes, wanted to make sure I
// understand the current behavior well first. There's a behavior in
// tests/math/unbalanced.typ that I feel isn't necessarily expected. That
// unclosed delimiters slurp up everything following them. See: `$1/(2$` `$1/( 2
// (x)` and the cursed `$1 /((1+1 /[1+1 /{1+1 /(1+1)$` (this also applies to
// super/sub-scripts). The problem is that this slurping of everything after
// them makes it hard to actually call functions on these objects, since the
// bottom-most paren eats the function-call's last parenthesis

fn math_unparen(p: &mut Parser, m: Marker) {
    // look at the node of this marker
    let Some(node) = p.nodes.get_mut(m.0) else { return };
    if node.kind() != SyntaxKind::MathDelimited {
        // if it's not a MathDelimited, we can't remove these parens
        return;
    }

    // if it IS MathDelimited, try to remove normal parentheses
    ...
    // then always convert the wrapping into a basic Math wrapper
    // (the parens will be removed properly later)
...
fn math_op(kind: SyntaxKind) -> Option<(SyntaxKind, SyntaxKind, ast::Assoc, usize)> {
    match kind {
        SyntaxKind::Underscore => {
            // this used to be 3, then was decreased to make it work better
